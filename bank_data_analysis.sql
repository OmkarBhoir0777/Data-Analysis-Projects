...
select * from finance_2csv;
select * from finance_1 as f1 join finance_2csv as f2 on f1.id = f2.id ;
...
from finance_1 as f1 join finance_2csv as f2 on f1.id = f2.id 
...
select 
  ...
  concat(round((sum(total_pymnt)/(
     ...
     join finance_2csv as f2 on f1.id = f2.id 
     ))*100,2),"%") as Percentage
...
join finance_2csv as f2 on f1.id = f2.id group by home_ownership, last_payment_d ;
...
count(*) as Total_number_of_customers from finance_1 as f1 join finance_2csv as f2 on f1.id = f2.id;
